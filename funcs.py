...
def cheat_algori(screen,choice,type,matrixidx): #choice=2 if dfs, 3 if A*; type=5 if hitori, 7 if hashi
    ...
    drawbutton(screen,"Statistic: Off",1000,250)
    drawbutton(screen,"Home",1000,350)
        ...
        for ev in pygame.event.get():
            if ev.type==MOUSEBUTTONDOWN:
                ...
                if (act>0 and not(pau)) or act==0 or act==3:
                    screen.blit(s_1,(1000,150+100*act))
                ...
                if act==0:
                    ...
                    if showstat:
                        drawbutton(screen,"Statistic: On",1000,250)
                        screen.blit(font.render(algo+"Time: "+str(runtime),False,white),(30,300))
                    else:
                        drawbutton(screen,"Statistic: Off",1000,250)
                        pygame.draw.rect(screen,black,(20,290,370,100))
                elif act==2:
                    return 4
                elif act==3:
                    sys.exit()       
